<h1>2020 - 1 cuatrimestre - PPS -- La Comanda</h1>

<h1 align="center"> :computer: 2020 - 1 cuatrimestre - PPS. </h1> 

<h1 align="center"> LA COMANDA </h1> 


<p align="center">
<img src="https://i.imgur.com/gvgMcG6.png" width="400" height="400">
</p>


<br>
<br>
<h1 align="center"> INTEGRANTES </h1> 

* **Cristian Luciano Moreno** ***** *Integrante Alfa* *****  **GITHUB:** [Luchomoreno](https://github.com/LuchoMoreno)
* **Veronica Florencia Bustos** ***** *Integrante Beta* ***** **GITHUB:** [Veronicabustoss](https://github.com/veronicabustoss)

<br>
<br>

## :bulb: Semana del 23/05 al 30/05


### Luciano 

- Fork repositorio. :white_check_mark:
- Icono, Splash estatico. :white_check_mark:
- Funcionalidad del lector QR DNI. :white_check_mark:
- Creación de servicios - QR Scanner + SpinnerService + ToastService. :white_check_mark:
- Generación de resources para la aplicación. :white_check_mark:
- Generación de primeros APK (no del todo funcionales). :white_check_mark:

### Verónica 

- Splash animado
- Base de datos con los usuarios (listo los usuarios). :white_check_mark:
- Creacion del login - Login funcional. :white_check_mark:
- Creacion de la pagina alta-supervisor. :white_check_mark:
- Creacion de la pagina alta-cliente (Si el cliente se registra como anonimo o cliente corriente). :x:

<h4></h4>


<p float="left">

<img src="https://i.imgur.com/6iJZZM4.png" width="300" height="500">
  
<img src="https://i.imgur.com/erVUmcX.png" width="300" height="500"> 

</p>


## :bulb: Semana del 31/05 al 06/06


### Luciano 

- Alta productos (código) + Parte visual. Se añaden distintos tipos de productos. :white_check_mark:
- Alta mesa (código) + Parte visual. Se añaden distintos tipos de mesas. :white_check_mark:
- Solución de un error al guardar las imagenes en firebase. :white_check_mark:
- Se agrega función que guarda las fotos con un nombre determinado. :white_check_mark:
- Detalles minimos en mejoras de código y nombres de funciones. :white_check_mark:

### Verónica 

- Alta cliente (código) + Parte visual (finalizado de la semana anterior). El cliente ahora puede ser anónimo. :white_check_mark:
- Alta empleado (código) + Parte visual. :white_check_mark:
- Alta supervisor (código) + Parte visual. :white_check_mark:
- SlideMenu : Creación de un nuevo slideMenu en /home. :white_check_mark:
- Servicio Database: Nuevo servicio generado, que se comunica con firebase. :white_check_mark:
- Primeras aproximaciones de notificaciones PUSH. :white_check_mark:

<h4></h4>


## :bulb: Semana del 07/06 al 13/06


### Luciano 

- Lógica de las notificaciones push, con cambios de estado en clientes [PENDIENTE / APROBADO]
- Mejoras en la función que envía CORREO al usuario registrado. Tenia un error mínimo. 
- Arreglos completos al agregar un usuario. Los mismos pueden ser aceptados o rechazados sin error.
- QR: Se crea el QR para pedir una mesa.
- Lista de espera Metre terminada -> El metre puede ver los usuarios que necesitan mesa.
- Generación de APKS para testeos generales.

### Verónica  

- Creación del código de las aplicaciones push. El cliente debe ser aprobado posterior a su registro.
- Creación del MENU para cargar clientes.
- CORREO: Se le envía un correo al usuario registrado.
- QR : Se crea el QR de cliente anónimo.
- Lista de espera Metre terminada -> El metre puede dar una mesa.

<h4></h4>


## :bulb: Semana del 14/06 al 20/06

## :bulb: Semana del 21/06 al 27/06


